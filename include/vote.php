...
class Vote {
    ...
    private function vote($ref, $productId, $client_id, $vote){
        ...
        $req = $this->pdo->prepare("SELECT id, vote from votes WHERE ref = :ref AND product_id = :product_id AND client_id = :client_id ");
        $req->execute([
            ':ref' => $ref,
    ...
    public function updateCount($ref, $productId){
        $req = $this->pdo->prepare("SELECT count(id) as count, vote from votes WHERE ref = :ref AND product_id = :product_id GROUP BY vote");
        $req->execute([
            ':ref' => $ref,
            ':product_id' => $productId, 
        ]); 
        ...
        $req = $this->pdo->prepare("UPDATE $ref SET like_count = {$counts[1]}, dislike_count = {$counts[-1]}  WHERE id = $productId");
        return true;
