...
if (!empty($_POST)) {
    ...
    if (empty($errors)) {
        ...
        // mail($_POST['email'], 'Confirmation de votre compte', "Afin de valider votre compte merci de cliquer sur ce
        ...
        <article class="card-body">
            ...
            <table class="table table-hover">
                ...
                <tbody>
                    ...
                    <?php
                        $requeteSelect = "SELECT p.id, p.name,p.quantity, p.price, p.description, p.ref, pimg.path, 
                            p.is_location,p.created_at, p.updated, pimg.is_main, p.category_id 
						FROM product AS p LEFT JOIN product_image AS pimg ON p.id = pimg.product_id ";
                        // $requeteSelect = "SELECT p.id, p.name, p.description, p.price, p.quantity, p.ref,
                        // p.is_location, p.created_at, pimg.is_main, pimg.path FROM product AS p
